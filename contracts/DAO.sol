...
// Adjusted from Fluidity Money's DAOStable slightly (ProposalStatus enum
// is here, and IERC20 is used as the voting denominator)
...
/*
 * DAO is used to power Make Adelaide's DAO, and assumes that the
 * associated token is soulbound
 */
contract DAO {
    ...
    function threePercentTotalSupply() internal view returns (uint256) {
        return token_.totalSupply() * 3 / 100;
    }
        ...
        public view returns (uint256)
    {
        ...
        uint256 available = token_.balanceOf(_spender);
